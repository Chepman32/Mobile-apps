...
const AnalyticsScreen = () => {
  ...
  useEffect(() => {
    db.transaction((tx) => {
      tx.executeSql(
        ...
        () => {
          tx.executeSql(
            'SELECT * FROM mood_entries ORDER BY date ASC',
            [],
            (_, { rows }) => {
              const loadedEntries = [];
  ...
  const getMoodData = () => {
    ...
    moodEntries.forEach(entry => {
      dailyMoods[entry.date] = (dailyMoods[entry.date] || 0) + entry.mood;
    });

    const sortedDates = Object.keys(dailyMoods).sort();
    const labels = sortedDates.slice(-7); // Last 7 days
    const data = labels.map(date => dailyMoods[date] / (moodEntries.filter(e => e.date === date).length || 1)); // Average mood
