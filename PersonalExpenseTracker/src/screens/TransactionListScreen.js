...
const TransactionListScreen = ({ navigation }) => {
  ...
  const loadTransactions = useCallback(() => {
    db.transaction((tx) => {
      tx.executeSql(
        'CREATE TABLE IF NOT EXISTS transactions (id INTEGER PRIMARY KEY AUTOINCREMENT, amount REAL, category TEXT, description TEXT, date TEXT)',
        [],
        () => {
          tx.executeSql(
  ...
  const purchasePremium = () => {
    Alert.alert(
      ...
      'In a real app, this would initiate an in-app purchase for custom categories, advanced analytics, etc.'
    );
  };
