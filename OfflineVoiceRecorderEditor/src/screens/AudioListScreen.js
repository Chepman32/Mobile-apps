    ...
    Alert.alert(
      ...
      [
        ...
        { text: 'Delete', onPress: async () => {
            try {
              await RNFS.unlink(path); // Delete file from storage
              db.transaction((tx) => {
                tx.executeSql(
                  ...
                  (tx, error) => console.error('Error deleting audio note from DB', error)
                );
              });
            } catch (error) {
  ...
  const renderAudioNote = ({ item }) => (
    <View style={styles.audioNoteItem}>
      ...
      <View style={styles.itemControls}>
        ...
        <TouchableOpacity style={styles.deleteButton} onPress={() => onDeleteAudio(item.id, item.path)}>
          <Text style={styles.buttonText}>Delete</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.editButton} onPress={() => navigation.navigate('Editor', { audioUri: item.path })}>
