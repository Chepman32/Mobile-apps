...
const GoalListScreen = ({ navigation }) => {
        ...
        () => {
          ...
          tx.executeSql(
            ...
            (_, { rows }) => {
              if (rows.length === 0) {
                const dummyGoals = [
                  { name: 'Lose 5kg', target: 5, current: 1, isPremium: 0 },
                  { name: 'Meditate Daily', target: 30, current: 10, isPremium: 0 },
                  { name: 'Run Marathon', target: 42.195, current: 5, isPremium: 1 },
                ];
  ...
  const purchasePremium = () => {
    Alert.alert(
      ...
      'In a real app, this would initiate an in-app purchase for advanced coordination, wellness coaching, etc.'
    );
  };
