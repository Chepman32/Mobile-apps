...
interface PocketPolyglotCLIState {
...
const initialState: PocketPolyglotCLIState = {
  translations: [],
  dictionary: [],
  conversations: [],
  flashcards: [],
...
function pocketPolyglotCLIReducer(state: PocketPolyglotCLIState, action: PocketPolyglotCLIAction): PocketPolyglotCLIState {
  switch (action.type) {
    case 'SET_LOADING':
...
interface PocketPolyglotCLIContextType {
  state: PocketPolyglotCLIState;
  dispatch: React.Dispatch<PocketPolyglotCLIAction>;
...
const PocketPolyglotCLIContext = createContext<PocketPolyglotCLIContextType | undefined>(undefined);

export function PocketPolyglotCLIProvider({ children }: { children: ReactNode }) {
  const [state, dispatch] = useReducer(pocketPolyglotCLIReducer, initialState);
  ...
  const value: PocketPolyglotCLIContextType = {
    state,
    dispatch,
    addTranslation,
    updateTranslation,
  ...
  return (
    <PocketPolyglotCLIContext.Provider value={value}>
      {children}
    </PocketPolyglotCLIContext.Provider>
  );
}

export function usePocketPolyglotCLI() {
  const context = useContext(PocketPolyglotCLIContext);
  if (context === undefined) {
    throw new Error('usePocketPolyglotCLI must be used within a PocketPolyglotCLIProvider');
  }
  return context;
} 